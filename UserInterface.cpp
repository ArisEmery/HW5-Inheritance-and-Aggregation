...
void UserInterface::add()
{
    ...
    if (m_subRegionType!=Region::RegionType::UnknownRegionType) {
        ...
        if (data != "") {
            ...
            if (region != nullptr) {
                /*
                if(m_subRegionType==Region::WorldType){
                    Region::rootRegion=region;
                }
                */
                m_currentRegion->add(*region);
...
void UserInterface::edit()
{
    ...
    if (input!="")
    {
        ...
        {
            //Region* region;
            Region* region = Region::create(m_currentRegion->subRegions[id-1].getType(), m_currentRegion->subRegions[id-1].getData());

            if (region!=nullptr)
            {
                std::cout << "Editing: ";
                region->display(std::cout, 0, false, *(m_currentRegion));
                editName(m_currentRegion->subRegions[id-1]);
                editPopulation(m_currentRegion->subRegions[id-1]);
                editArea(m_currentRegion->subRegions[id-1]);
                /*
                editName(region);
...
void UserInterface::editName(Region &region)//change the parameter, be careful
{
    std::string updatedName = getStringInput("Enter an updated name (<enter> to keep current value):");
    if (updatedName!="")
    {
        region.setName(updatedName); //change to .
...
void UserInterface::remove()
{
    ...
    if (input!="")
    {
        ...
        if (valid && id>0)
        {
            m_currentRegion->subRegions.erase(m_currentRegion->subRegions.begin() + (id-1));
            for(int i=0;i<m_currentRegion->subRegions.size();i++){
                //int newID=(m_currentRegion->subRegions[i].getId()-1);
                m_currentRegion->subRegions[i].setId(i+1);
            }
...
{
    m_currentRegion->display(std::cout, 0, true, *m_currentRegion);
};
...
void UserInterface::changeToSubRegion()
{
    ...
    if (input!="")
    {
        ...
        {
            Region* region = Region::create(m_currentRegion->subRegions[id-1].getType(), m_currentRegion->subRegions[id-1].getData());
            //std::string data=m_currentRegion->subRegions[id-1].getData();
            //Region *region = Region::create(m_subRegionType, data);
            if (region!=nullptr)
            {
