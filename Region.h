...
public:
    ...
    int numSubRegions;
    Region getSubRegionByIndex(int x){return subRegions[x];};
    std::vector<Region> subRegions;
    std::string regionData; //Just to hold the data as a string for future use
    std::string getData();//getter method for the regionData string
    void setId(int x);
    int getSubRegionCount(){ return subRegions.size();};
    void addSubRegion(Region region);
    //static Region* rootRegion;
    ...
    unsigned int computeTotalPopulation(Region region);

    void list(std::ostream& out);
    void display(std::ostream& out, unsigned int displayLevel, bool showChild, Region region);
    void save(std::ostream& out, Region);
